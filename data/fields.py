...
FIELD_DEFINITIONS: OrderedDict[str, dict] = OrderedDict([
    ...
    ("explicit_act", {
        "group": 'Setting & Shot',
        # The act sentence is the pool's voice: every entry is a participial
        # phrase the engine drops onto "She is ..." (or "He is ...") in every
        # wardrobe tier. The camera-POV fetish acts use community canon --
        # 'face sitting' (weight rolling), 'golden shower' (urination on the
        # viewer), 'spit play' / 'spit shined', 'drooling', 'milking',
        # 'squirting', 'lactation' -- so the wording is what image models were
        # trained to associate with each practice.
        "female_options": [
            'no explicit action',
            'fingering herself',
        ...
        "optional": False,
        # act appears on roughly a third of random draws, never by accident.
        'weights': {
            'fingering herself': 0.5,
            'riding a fluffy pillow': 0.5,
            ...
            'blowing hot breath on the camera, a long slow exhale': 0.5,
            'drooling, thick saliva trailing from her lips onto the camera': 0.5,
            'kissing the camera open-mouthed, tongue slow': 0.5,
            'biting the camera firmly, teeth sinking in': 0.5,
            'moaning loudly at the camera, voice rough': 0.5,
            'clenching her breast and milking through her knuckles': 0.5,
            'sucking her own nipple hard into a peak': 0.5,
            'bouncing her breasts, each bounce heavy': 0.5,
            'pressing a thin stream of breast milk onto the camera': 0.5,
            'dripping, each drop landing on the camera': 0.5,
