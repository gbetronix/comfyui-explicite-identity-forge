...
- **Works (v2.0.0 rework):**
  ...
  - **Explicit action** — `explicit_act` field, 32 sentence-level acts + the neutral "no explicit action" (which stays in the JSON on purpose: a vault round-trip must replay the document byte-identically at a new seed, and an explicit neutral is the difference between "the document decided" and "a fresh draw picked something different"). Prose skips the neutral. **Breast/vagina acts (the field's `implies_fully_nude` list) resolve `Fully nude` — the act dictates the wardrobe; a locked costume still wins. Face/hands/saliva acts are tier-neutral.**
...
- **`scripts/render_config.json`** (gitignored) overrides the committed workflow's model/sampler/size settings for this machine's ComfyUI instance. It does not exist on a fresh clone; the script falls back to `gallery/cosplay/Krea2_ExpliciteIdentityForge_CharacterCycle.json`.
- **After a node schema change:** also run `python scripts/dump_frontend_fixtures.py` and commit the refreshed `tests/frontend/fixtures/nodes.json`. Run it with plain `python`, never with a real ComfyUI on `sys.path` — `ExpliciteIdentityForgeVaultLoad` would then list the user's saved characters and commit them.
