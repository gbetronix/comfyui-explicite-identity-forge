"""Generate manifest.json for the FETISH gallery.

    python gallery/fetish/build_manifest.py --images <dir>

=============================================================================
 THIS FILE IS ONE OF THREE NEAR-IDENTICAL COPIES
 gallery/fetish/build_manifest.py - gallery/archetypes/build_manifest.py -
...
GALLERY_KIND = "fetish"
VERSIONS_KIND = "fetish"

import sys as _sys
_sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from fetish_roster import entries as _entries, entry_names as _roster_names  # noqa: E402

#: The roster is data-driven (the engine's own explicit_act pool), so there are no sentinel names to filter out.
SENTINEL_NAMES = set()


def entry_names() -> list[str]:
    """Every act the engine can pick, by display name (dropdown + filename)."""
...
def entry_meta(name: str) -> dict:
    ...
    data = _entries()[name]
    return {"gender": "female", "group": "nude play" if data.get("nude_act") else "camera play"}
# ============================================================================
